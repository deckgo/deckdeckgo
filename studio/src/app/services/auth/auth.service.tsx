      ...
      firebase.auth().onAuthStateChanged(async (firebaseUser: FirebaseUser) => {
        if (!firebaseUser) {
          ...
          const authUser: AuthUser = {
            ...
            gitHub: firebaseUser.providerData?.[0]?.providerId === 'github.com' ?? false,
          };

          // Update anonymous user
          // Reference: https://github.com/firebase/firebaseui-web/issues/449
          if (!authUser.name && firebaseUser.providerData?.[0]?.displayName) {
            authUser.name = firebaseUser.providerData[0].displayName;
          }

          if (!authUser.photo_url && firebaseUser.providerData?.[0]?.photoURL) {
