...
import { downloadStarterMarkdown, downloadStarterMaster } from './download';
    ...
    try {
        const questions = [
            ...
            },
            {
                type: 'list',
                name: 'editing',
                message: 'What method do you want to use to edit your presentation?',
        ...
        inquirer.prompt(questions).then(async (answers: Answers) => {
            ...
            console.log('\nTo develop your slides, edit ' + cyan(answers.folder + (answers.editing === MARKDOWN ? '/src/index.md' : '/src/index.html')) + '\n');

            console.log('Run ' + cyan('npm run start') + ' in the newly created folder ' + cyan(answers.folder) + ' to serve your presentation locally at the address ' + cyan('http://localhost:3000') + '\n');

            console.log('Find this presentation in the remote control with the keyword: ' + cyan(answers.title) + '\n');
            console.log('If you rather like not to use the remote control while developing your deck, run ' + cyan('npm run start-no-remote') + ' instead of the previous command\n');
...
function downloadInstallPresentation(answers: Answers) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // 2. Download starter
            const buffer = await (answers.editing === MARKDOWN ? downloadStarterMarkdown() : downloadStarterMaster());
...
function replaceAnswers(answers: Answers) {
    const replaceResources = [answers.folder + '/src/index.html', answers.folder + '/src/manifest.json', answers.folder + '/webpack.config.js'];

    if (answers.editing === MARKDOWN) {
        replaceResources.push(answers.folder + '/src/index.md');
    }
