...
export function downloadStarterMaster() {
    return downloadFromURL(`https://github.com/deckgo/deckdeckgo-starter/archive/master.zip`);
}

export function downloadStarterMarkdown() {
    return downloadFromURL(`https://github.com/deckgo/deckdeckgo-starter/archive/markdown.zip`);
}
...
export function downloadFont(family: string, variants: string[], subsets: string[], userAgent: string) {
    return downloadFromURL(`https://fonts.googleapis.com/css?family=${family}:${variants.join(',')}&subset=${subsets.join(',')}`, {headers: {'User-Agent': userAgent}});
}
