    ...
    return new Promise<FontCss>(async (resolve, reject) => {
        ...
        try {
            if (font && fontSubsets && fontSubsets.length > 0 && fontVariants && fontVariants.length > 0) {
                ...
                promises.push(downloadFontCSS(font.family, fontVariants, fontSubsets, 'Mozilla/5.0 (X11; CrOS x86_64 8172.45.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.64 Safari/537.36'));
                promises.push(downloadFontCSS(font.family, fontVariants, fontSubsets, 'Mozilla/5.0 (Windows; U; MSIE 9.0; Windows NT 9.0; en-US)'));
                promises.push(downloadFontCSS(font.family, fontVariants, fontSubsets, 'Mozilla/5.0 (Macintosh; U; PPC Mac OS X 10_5_2; en-gb) AppleWebKit/526+ (KHTML, like Gecko) Version/3.1 iPhone'));
                promises.push(downloadFontCSS(font.family, fontVariants, fontSubsets, 'Mozilla/5.0 (iPhone; U; CPU like Mac OS X; en) AppleWebKit/420+ (KHTML, like Gecko) Version/3.0 Mobile/1A543 Safari/419.3'));
...
function filterFonts(fontsList: FontsList) {
    return new Promise<Font[]>(async (resolve, reject) => {
        try {
            ...
            const filteredFonts: Font[] = fontsList.items.filter((filteredFont: Font) =>
                filteredFont.family && filteredFont.family.toLowerCase().indexOf(answer.fontName.toLowerCase()) > -1);
