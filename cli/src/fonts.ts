      ...
      if (answer.font) {
        ...
        if (fontsList && fontsList.items && fontsList.items.length > 0) {
          ...
          if (font) {
            ...
            const fontCss: FontCss = await downloadFontAllCSS(
              font,
              fontVariants,
              fontSubsets
            );

            const localFontCss: FontCss = await downloadInstallFont(
...
function writeCssFiles(folder: string, font: Font, fontCss: FontCss) {
  return new Promise<void>(async (resolve, reject) => {
    ...
    try {
      await createFontCssFile(
        folder + "/src/css/google-fonts.css",
        font.family,
        fontCss
      );
...
function downloadFontCSS(
  ...
  return new Promise<string>(async (resolve, reject) => {
    try {
      ...
      const buffer: Buffer = await downloadFont(
        fontFamily,
        fontVariants,
        fontSubsets,
        userAgent
...
function selectFontSubsets(font: Font) {
  return new Promise<string[]>(async (resolve, reject) => {
    try {
      ...
      const answer: FontSubsetsAnswer = await inquirerFonts.prompt(
        questionFont
      );
...
function selectFontVariants(font: Font) {
  return new Promise<string[]>(async (resolve, reject) => {
    try {
      ...
      const answer: FontVariantsAnswer = await inquirerFonts.prompt(
        questionFont
      );
...
function chooseFont(filteredFonts: Font[]) {
  return new Promise<FontSelection>(async (resolve, reject) => {
    try {
      ...
      const msg: string =
        names.length > 0 ? "Select the font" : "No font found";
      ...
      if (answer && answer.fontName === "Search again") {
        ...
        const findFont: Font = filteredFonts.find((filteredFont: Font) => {
          return (
            ...
            filteredFont.family
              .toLowerCase()
              .indexOf(answer.fontName.toLowerCase()) > -1
          );
        });
